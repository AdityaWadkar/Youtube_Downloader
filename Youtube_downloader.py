...
file_size=0
# https://www.youtube.com/watch?v=npeYPND0cgU
def unit(size):
    ...
    if len(url) == 0:
        ...
        try:
            if "youtube" not in url:
                ...
                length=ob.length
                if length <= 60:
                    Video_length = Label(text=f"Video Length : {length} Seconds", font=("jost", 12))
                    Video_length.pack(side=TOP, fill=X, pady=15)
                else:
                    mins=convert(length)
                    Video_length = Label(text=f"Video Length : {mins} Minutes", font=("jost", 12))
                    Video_length.pack(side=TOP, fill=X, pady=15)
            ...
            showinfo("Download error ", "This is not a Valid Youtube video link!!!")
            btn.config(text="Download")
            btn.config(state=NORMAL)
            urlfield.config(state=NORMAL)
            urlfield.delete(0, END)
